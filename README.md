...
当多个运算符同时出现在一个表达式时候，要按照运算符优先级顺序，先乘除，后加减，有小括号优先的的顺序
...
在python中如果获取一个不存在的值，系统会报错，如果用get方法来获取不存在的值则不会报错而是会返回一个none值，也可以给get方法设定一个返回值，如果get方法获取有返回值，就返回当前存在的值，否则返回设定值
